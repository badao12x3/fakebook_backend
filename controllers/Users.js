...
usersController.edit = async (req, res, next) => {
    try {
        ...
        // new:true -option to return a new document
        // Validation là một middleware, Mongoose thực thi middleware này như là pre('save') trong một schema một cách tự động
...
usersController.changePassword = async (req, res, next) => {
    try {
        ...
        user = await UserModel.findOneAndUpdate({_id: userId}, {password: hashedNewPassword}, {
...
// chưa hiểu lắm ý đồ, trông 2 method khá giống nhau
...
usersController.setBlockDiary = async (req, res, next) => {
    try {
        ...
        let user = await UserModel.findById(req.userId);
        blocked = []
        // issue by DaoMinhTien: NOT CHECKED: var userObject = user.toObject(); if(userObject.hasOwnProperty('verified'))
        if (user.hasOwnProperty('blocked')) {
